...
class SquareAttack():
    ...
    def margin_and_loss(self, x):
        """
        :param y:        correct labels if untargeted else target labels
        """

        sampled_image_latents, target_image_latents = self.predict(x)
        # print(torch.min(sampled_image_latents))
        # print(torch.max(sampled_image_latents))
        # print(torch.min(target_image_latents))
        # print(torch.max(target_image_latents))
        # input("check check")
        loss = (sampled_image_latents - target_image_latents).norm(p=2)
    ...
    def attack_single_run(self, x):
        with torch.no_grad():
            ...
            if self.norm == 'Linf':
                x_best = x + self.eps * self.random_choice(
                    [x.shape[0], c, 1, w]) * (self.predict.latent_mask < 0.5)
                loss_min = self.margin_and_loss(x_best)
                n_queries = torch.ones(x.shape[0]).to(self.device)
                s_init = int(math.sqrt(self.p_init * n_features / c))

                from torchvision.utils import save_image
                for j in range(x_best.shape[0]):
                    save_img = x_best[j, :3]
                    print(save_img.shape)
                    save_img = (save_img - torch.min(save_img)) / (torch.max(save_img) - torch.min(save_img))
                    save_image(save_img, './x_init{}.png'.format(j))
                
                # if (margin_min < 0.0).all():
                #     return n_queries, x_best
                i_iter = 0
                all_improved_counter = 0
                # for i_iter in tqdm(range(self.n_queries)):
                while i_iter < self.n_queries:
                    # print(counter)
                    # counter += 1
                    ...
                    # print("vh, vw, s", vh, vw, s)
                    
                    x_new = x_best_curr + new_deltas * (self.predict.latent_mask < 0.5)
                    x_new = torch.min(torch.max(x_new, x_curr - self.eps), x_curr + self.eps)
                    # print('norm:', (x_new - x_curr).norm(p=2))
                    # x_new = torch.clamp(x_new, 0., 1.)
                    x_new = self.check_shape(x_new)
                    if torch.sum(torch.abs(x_new - x_best_curr)) > 0:
                        if i_iter % 5 == 0:
                            print(i_iter, loss_min_curr.item(), all_improved_counter)
                        i_iter += 1
                    else:
                        continue
                    
                    loss = self.margin_and_loss(x_new)

                    # update loss if new loss is better
                    idx_improved = (loss < loss_min_curr).float()
                    if idx_improved > 0:
                        all_improved_counter += 1

                    # print(loss, loss_min_curr)
                    # input("check")
                    # print("min_max_x_new", torch.min(x_new), torch.max(x_new))
                    # print("min_max_x_curr", torch.min(x_curr), torch.max(x_curr))

                    # if torch.sum(x_new - x_best_curr) > 0:
                    ...
                    #     #     print(save_img.shape)
                    #     #     save_img = (save_img - torch.min(save_img)) / (torch.max(save_img) - torch.min(save_img))
                    #     #     save_image(save_img, './x_new{}.png'.format(j))

                    #     # for j in range(x.shape[0]):
                    #     #     save_img = x[j, :3]
                    #     #     print(save_img.shape)
                    #     #     save_img = (save_img - torch.min(save_img)) / (torch.max(save_img) - torch.min(save_img))
                    #     #     save_image(save_img, './x_{}.png'.format(j))

                    #     # for j in range(x_best.shape[0]):
                    #     #     save_img = x_best[j, :3]
                    #     #     print(save_img.shape)
                    #     #     save_img = (save_img - torch.min(save_img)) / (torch.max(save_img) - torch.min(save_img))
                    #     #     save_image(save_img, './x_best{}.png'.format(j))
                    #     #     input("check")
                    ...
                    # print("improved: ", idx_improved)
                    # print(idx_improved.shape)
    ...
    def perturb(self, x):
        ...
        for counter in range(self.n_restarts):
            ...
            if True:
                ...
                _, adv_curr = self.attack_single_run(x_to_fool)
